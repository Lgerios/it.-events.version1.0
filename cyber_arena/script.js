    ...
    tournaments.forEach(t => {
        ...
        card.innerHTML = `
            ...
            <div style="display: flex; gap: 10px; margin: 1rem;">
                ...
                <button class="button button--secondary" onclick="showBracket(${t.id})">Сетка</button>
            </div>
        `;
...
// ---------- ТУРНИРНАЯ СЕТКА ----------
window.showBracket = async function(tournamentId) {
    const container = document.getElementById('bracket-container');
    container.innerHTML = '<p>Загрузка сетки...</p>';

    try {
        const [tournament, participants, users] = await Promise.all([
            fetch(`${API_BASE_URL}/tournaments/${tournamentId}`).then(res => res.json()),
            fetch(`${API_BASE_URL}/participants?tournamentId=${tournamentId}`).then(res => res.json()),
        ...
        if (players.length === 0) {
            container.innerHTML = '<p>На этот турнир ещё никто не зарегистрировался.</p>';
            return;
        }

        const rounds = generateBracketRounds(players);
        renderBracket(container, tournament.title, rounds);
    } catch (error) {
        console.error(error);
        container.innerHTML = '<p>Ошибка загрузки сетки.</p>';
    }
};

function generateBracketRounds(players) {
    let size = 1;
    ...
    while (current.length > 1) {
        ...
        if (roundNumber === totalRounds) {
            ...
            const fraction = Math.pow(2, totalRounds - roundNumber + 1);
...
function renderBracket(container, title, rounds) {
    let html = `<h3 style="margin-bottom: 1.5rem;">Сетка турнира: ${title}</h3>`;
    html += '<div class="bracket">';

    rounds.forEach(round => {
        html += `<div class="bracket__round">`;
        html += `<h4 class="round-title">${round.name}</h4>`;
        round.matches.forEach(match => {
            html += `<div class="match">`;
            html += `<span class="match__team">${match.team1}</span>`;
            html += `<span class="match__score">${match.score}</span>`;
            html += `<span class="match__team">${match.team2}</span>`;
            html += `</div>`;
        });
        html += `</div>`;
    });
